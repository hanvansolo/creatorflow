...
const DEFAULT_JOBS = [
  ...
  { name: 'data-sync',         path: '/api/cron/{secret}/data-sync',           intervalMs: 24 * 60 * 60 * 1000, delayMs: 120_000  }, // Once daily — priority leagues only (~80 calls). Use ?full=true manually for all 133
  ...
  { name: 'live-sync',          path: '/api/cron/{secret}/live-sync',          intervalMs: 10 * 60 * 1000,      delayMs: 60_000   }, // Every 10 min - live match scores, events, stats + AI analysis
